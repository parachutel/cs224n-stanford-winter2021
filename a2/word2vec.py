...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    outsideSoftmaxProbVec = softmax(outsideVectors @ centerWordVec)
    loss = -np.log(outsideSoftmaxProbVec[outsideWordIdx])
    gradCenterVec = - outsideVectors[outsideWordIdx] \
        + outsideVectors.T @ outsideSoftmaxProbVec
    gradOutsideVecs = np.reshape(outsideSoftmaxProbVec, (-1, 1)) \
        @ np.reshape(centerWordVec, (1, -1))
    gradOutsideVecs[outsideWordIdx] = \
        (outsideSoftmaxProbVec[outsideWordIdx] - 1) * centerWordVec # w != o
    ### END YOUR CODE
