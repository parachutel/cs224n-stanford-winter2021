...
class QANetXL(nn.Module):
    ...
    def _forwardEmb(self, word_emb, mask, mem=None):
        ...
        core_out = self.emb_enc(core_out, mask, l=1, blks=1, r=pos_emb, 
            r_w_bias=self.r_w_bias, r_r_bias=self.r_r_bias, dec_attn_mask=None, mems=mem)
        core_out = self.drop(core_out)

        return core_out

    def _forwardEnc(self, word_emb, mask, mem=None):
        bsz, d_model, seg_len = word_emb.size()

        mlen = mem.shape[0] if mem is not None else 0
        assert mlen == 0 or mlen == self.mem_len
        aug_len = mlen + seg_len

        pos_seq = torch.arange(aug_len - 1, -1, -1.0, device=word_emb.device, 
                               dtype=word_emb.dtype) # aug_len
        if self.clamp_len > 0:
            pos_seq.clamp_(max=self.clamp_len)
        pos_emb = self.pos_emb(pos_seq)
        ...
        for i, layer in enumerate(self.model_enc_blks):
            core_out = layer(core_out, mask, l=i*(2+2)+1, blks=7, r=pos_emb, 
                r_w_bias=self.r_w_bias, r_r_bias=self.r_r_bias, dec_attn_mask=None, mems=mem)
    ...
    # Intra-context segmentation
    def forward(self, Cword, Cchar, Qword, Qchar, mems=None):
        ...
        for i_seg in range(n_segments):
            ...
            M0 = self.cq_resizer(X) # (bs, d_model, full_seq_len)
